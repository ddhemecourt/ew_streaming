...
void* pdw_constructor(char* pdw_word, struct pdw_s pdw,int inc, int pdw_type){
	...
	//PDW Type Basic is 1
		...
		//header
		pdw_word[0+inc] = (TOA >> 36) & (0xFF);	
		pdw_word[1+inc] = (TOA >> 28) & (0xFF);
		pdw_word[2+inc] = (TOA >> 20) & (0xFF);
		pdw_word[3+inc] = (TOA >> 12) & (0xFF);
		pdw_word[4+inc] = (TOA >> 4) & (0xFF);
		pdw_word[5+inc] = (TOA << 4)|(SEG << 3)|(0x00);
		
		//flags
		pdw_word[6+inc] = (0x00)|(pdw.M3 << 2)|(pdw.M2 << 1)|(pdw.M1);

		//body
		pdw_word[7+inc] =  (freq_offset >> 24) & (0xFF);
		pdw_word[8+inc] =  (freq_offset >> 16) & (0xFF);
		pdw_word[9+inc] = (freq_offset >> 8) & (0xFF);
		pdw_word[10+inc] = (freq_offset) & (0xFF);
		pdw_word[11+inc] = (level_offset >> 8) & (0xFF);
		pdw_word[12+inc] = (level_offset) & (0xFF);
		pdw_word[13+inc] = (phase_offset >> 8) & (0xFF);
		pdw_word[14+inc] = (phase_offset) & (0xFF);

		
		//payload
		if(pdw.MOD_TYPE == 0){
			pdw_word[15+inc] = (0x00)|(TON >> 40);
			pdw_word[16+inc] = (TON >> 32) & (0xFF);
			pdw_word[17+inc] = (TON >> 24) & (0xFF);
			pdw_word[18+inc] = (TON >> 16) & (0xFF);
			pdw_word[19+inc] = (TON >> 8) & (0xFF);
			pdw_word[20+inc] = (TON) & (0xFF);
			pdw_word[21+inc] = 0x00;
			pdw_word[22+inc] = 0x00;
			pdw_word[23+inc] = 0x00;
			pdw_word[24+inc] = 0x00;
			...
			pdw_word[15+inc] = (0x01 << 4)|(0x00);
			pdw_word[16+inc] = 0x00;
			pdw_word[17+inc] = (0x00)|(TON >> 24);
			pdw_word[18+inc] = (TON >> 16) & (0xFF);
			pdw_word[19+inc] = (TON >> 8) & (0xFF);
			pdw_word[20+inc] = (TON) & (0xFF);
			freq_inc = (unsigned long)(pdw.FREQ_INC)/TON/(2.4e9)*pow(2,64);
			pdw_word[21+inc] = (freq_inc >> 56) & (0xFF);
			pdw_word[22+inc] = (freq_inc >> 48) & (0xFF);
			pdw_word[23+inc] = (freq_inc >> 40) & (0xFF);
			pdw_word[24+inc] = (freq_inc >> 32) & (0xFF);
			pdw_word[25+inc] = (freq_inc >> 24) & (0xFF);
			pdw_word[26+inc] = (freq_inc >> 16) & (0xFF);
			pdw_word[27+inc] = (freq_inc >> 8) & (0xFF);
			pdw_word[28+inc] = (freq_inc) & (0xFF);
			pdw_word[29+inc] = 0x00;
			pdw_word[30+inc] = 0x00;
			pdw_word[31+inc] = 0x00;
			...
			chip_width = pdw.CHIP_WIDTH*(1e-6)*(2.4e9);
			pdw_word[19+inc] = (0x03 << 4)|(chip_width >> 40);
			pdw_word[20+inc] = (chip_width >> 32) & (0xFF);
			pdw_word[21+inc] = (chip_width >> 24) & (0xFF);
			pdw_word[22+inc] = (chip_width >> 16) & (0xFF);
			pdw_word[23+inc] = (chip_width >> 8) & (0xFF);
			pdw_word[24+inc] = (chip_width) & (0xFF);
			pdw_word[25+inc] = (pdw.CODE << 4)|(0x00);
			pdw_word[26+inc] = 0x00;
			pdw_word[27+inc] = 0x00;
			pdw_word[29+inc] = 0x00;
			pdw_word[30+inc] = 0x00;
